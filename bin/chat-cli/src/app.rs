...
fn decoded_display(mc: message_types::MessageContent) -> (String, Option<String>) {
    ...
    match mc {
        Text { body } => (body, None),
        Markdown { body } => (format!("[md] {body}"), None),
        Reply { in_reply_to, body } => (body, Some(in_reply_to)),
        Reaction { in_reply_to, emoji } => (format!("reacted {emoji}"), Some(in_reply_to)),
    ...
    S: ConversationStore + Send,
{
    ...
    pub fn send_message(&mut self, content: &str) -> Result<()> {
        let encoded = message_types::encode(&message_types::MessageContent::text(content))
    ...
    fn send_markdown(&mut self, content: &str) -> Result<()> {
        let encoded = message_types::encode(&message_types::MessageContent::markdown(content))
            .map_err(|e| anyhow::anyhow!("encode content: {e}"))?;
        self.send_encoded(encoded, format!("[md] {content}"))
    }

    /// Send a reply referencing message id `in_reply_to`.
    fn send_reply(&mut self, in_reply_to: &str, content: &str, preview: &str) -> Result<()> {
        let encoded =
            message_types::encode(&message_types::MessageContent::reply(in_reply_to, content))
                .map_err(|e| anyhow::anyhow!("encode content: {e}"))?;
...
mod tests {
    ...
    use message_types::{MessageContent, decode, encode};
    ...
    fn renders_text() {
        assert_eq!(
            body_of(&encode(&MessageContent::text("hi there")).unwrap()),
            "hi there"
        );
    }
    ...
    fn tags_markdown() {
        assert_eq!(
            body_of(&encode(&MessageContent::markdown("# H")).unwrap()),
            "[md] # H"
        );
    }
    ...
    fn reply_exposes_its_target() {
        let (body, target) = decoded_display(
            decode(&encode(&MessageContent::reply("abc123", "sure")).unwrap()).unwrap(),
        );
        assert_eq!(body, "sure");
