//! A store that refuses a write or a read on demand, so a test can see what an operation leaves
//! behind when the store does not answer it.
...
impl Faults {
    ...
    /// While set, reading a conversation's record fails.
    pub fn fail_load(&self, fail: bool) {
        self.load.set(fail);
    }

    /// How many transactions the stores built from these faults have opened.
    pub fn begins(&self) -> usize {
        self.begins.get()
    }
}

/// An in-memory store that refuses what its [`Faults`] name.
...
impl KvStore for FaultStore {
    fn begin(&self) -> Result<Box<dyn KvTx + '_>, StorageError> {
        self.faults.begins.set(self.faults.begins.get() + 1);
        Ok(Box::new(FaultTx {
...
impl ConversationStore for FaultStore {
    ...
    fn load_conversation(
        ...
        if self.faults.load.get() {
            return Err(StorageError::Database("load refused".into()));
        }
