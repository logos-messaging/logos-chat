...
//! not serve one whose transaction was rolled back: a peer fetching it would send a welcome Saro
//! cannot open. In the other direction, a conversation whose transaction landed is real, so a
//! publish that fails afterwards costs a message, never the conversation.
...
#[test]
fn a_key_package_is_not_registered_when_its_transaction_does_not_land() {
...
#[test]
fn a_conversation_survives_a_publish_that_fails_after_its_transaction_landed() {
    let ds = LocalBroadcaster::new();
    let rs = EphemeralRegistry::new();
    ...
    let convos = saro.list_all_conversations().unwrap();
