...
use crate::storage::{ConversationMeta, ConversationStore, Scope};
...
impl<'a, S: ExternalServices + 'static> Core<S> {
    ...
    pub fn remove_conversation(&mut self, convo_id: &str) -> Result<(), ChatError> {
        let kind = Self::stored_kind(&self.store, convo_id)?;
    ...
    /// The kind the store's record names for a conversation.
