...
/// The OpenMLS storage provider over one conversation's scope, key packages excepted: those live in
/// [`KeyPackages`], outside every conversation, since a welcome of any kind consumes one.
///
/// Group state carries no group id in its keys, since the scope already names the conversation.
/// Each variant holds the destinations one kind of operation has, and reaching for the destination
/// a variant lacks is an error, not a panic.
pub(crate) enum MlsAdapter<'a> {
    /// A group operation, which never mints or consumes a key package.
    Convo(ScopedKvStore<'a>),
    /// Minting a key package, or reading a welcome for the group id it carries before that
    /// conversation exists.
    KeyPackages(KeyPackages<'a>),
    /// A join that reads the key package and writes the group in one call.
    Join(ScopedKvStore<'a>, KeyPackages<'a>),
}

impl<'a> MlsAdapter<'a> {
    fn convo(&self) -> Result<ScopedKvStore<'a>, MlsStorageError> {
        match self {
            Self::Convo(convo) | Self::Join(convo, _) => Ok(*convo),
            Self::KeyPackages(_) => Err(MlsStorageError::Unscoped("mls group state")),
        }
    }

    fn key_packages(&self) -> Result<KeyPackages<'a>, MlsStorageError> {
        match self {
            Self::KeyPackages(key_packages) | Self::Join(_, key_packages) => Ok(*key_packages),
            Self::Convo(_) => Err(MlsStorageError::Unscoped("mls key packages")),
        }
    }
