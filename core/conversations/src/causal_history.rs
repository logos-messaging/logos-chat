...
struct ConvoState {
    ...
    acked_by: HashMap<String, HashSet<Signer>>,
}
...
impl CausalHistoryStore {
    ...
    pub fn on_receive(
        ...
        let Ok(payload_sender) = Signer::try_from(payload.sender_id.as_str()) else {
        ...
        for entry in &payload.causal_history {
            let Ok(entry_sender) = Signer::try_from(entry.sender_id.as_str()) else {
...
mod tests {
    ...
    fn each_peer_acknowledges_separately() {
        ...
        let holders: Vec<Signer> = alice_store
            .take_acks()
            .into_iter()
