...
impl GroupV2Convo {
    ...
    pub fn new<S: ExternalServices>(
        ...
        let conversation = Conversation::create(
            &convo_id,
            &MlsProvider::new(&service_ctx.crypto, MlsAdapter::Convo(kv)),
    ...
    pub fn new_from_welcome<S: ExternalServices>(
        ...
        let Some(conv) = Conversation::join(
            &MlsProvider::new(&service_ctx.crypto, MlsAdapter::Join(kv, key_packages)),
            &service_ctx.mls_identity,
    ...
    pub fn welcome_convo_id<S: ExternalServices>(
        ...
        let processed = ProcessedWelcome::new_from_welcome(
            &MlsProvider::new(&service_ctx.crypto, MlsAdapter::KeyPackages(key_packages)),
    ...
    S: ExternalServices,
{
    ...
    fn handle_frame(
        ...
        let provider = MlsProvider::new(&service_ctx.crypto, MlsAdapter::Convo(kv));
    ...
    fn wakeup(
        ...
        let poll_outcome = self.conversation.poll(
            &MlsProvider::new(&ctx.crypto, MlsAdapter::Convo(kv)),
            &ctx.mls_identity,
        );
    ...
    S: ExternalServices,
{
    ...
    fn remove_member(
        ...
        for target in &targets {
            match self
                .conversation
                .remove_member(&provider, &service_ctx.mls_identity, target)
            {
                Ok(()) => {}
